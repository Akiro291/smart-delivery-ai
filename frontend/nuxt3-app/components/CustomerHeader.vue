<template>
  <header class="sticky top-0 z-20 bg-white border-b border-gray-200 h-16 flex items-center px-4 gap-4">
    ...
    <h1 class="text-xl font-semibold text-gray-800">Клиентский кабинет</h1>
    <div class="flex items-center gap-4 ml-auto">
      <div class="relative group">
        ...
        <div class="hidden group-hover:block absolute right-0 mt-2 w-48 bg-white rounded shadow-lg border z-50">
          <NuxtLink to="/customer/profile" class="block px-4 py-2 hover:bg-gray-50">Профиль</NuxtLink>
...
const userName = localStorage.getItem('user_name') || 'Клиент'
const initial = computed(() => (userName || 'C')[0]?.toUpperCase() || 'C')
