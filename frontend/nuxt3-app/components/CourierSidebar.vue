<template>
  ...
  <div :class="['fixed inset-y-0 left-0 z-50 w-64 bg-green-900 transform transition-transform duration-300 ease-in-out lg:hidden', mobileOpen ? 'translate-x-0' : '-translate-x-full']">
    <div class="flex items-center justify-between px-4 h-16 border-b border-green-700">
  ...
  <aside :class="['fixed inset-y-0 left-0 z-30 bg-green-900 transition-all duration-300 ease-in-out hidden lg:flex lg:flex-col', collapsed ? 'w-16' : 'w-64']">
    <div class="flex items-center justify-between px-4 h-16 border-b border-green-700">
...
const menuItems: MenuItem[] = [
  { label: 'Мои заказы', to: '/courier', icon: '\uD83D\uDCE6' },
  { label: 'Доставки', to: '/courier/orders', icon: '\uD83D\uDE9A' },
